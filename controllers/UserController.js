...
export const VerifyUserLink = async(req,res)=>{
    try{
        ...
        if(!find_User){
            ...
            const link = `http://localhost:5000/users/verifyUser/${find_User._id}/${ResetString}/${token}`
...
export const ForgotPassword = async(req,res)=>{
    try{
        ...
        if(!find_User){
            ...
            const link = `http://localhost:5000/users/ResetPassword/${find_User._id}/${ResetString}/${token}`
            // for Sending mails - nodemailer
            var transporter = nodemailer.createTransport({
                service: 'gmail',
                auth: {
